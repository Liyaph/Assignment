...
math_operation <- function(num1, num2, oper){
  result <- switch(oper,
                    ...
                    "subtract" = paste(num1, "-", num2, "=", num1 - num2),
                    "multiply" = paste(num1, "/", num2, "=", num1 * num2),
                    "divide" = paste(num1, "/", num2, "=", num1 / num2),
                    "Error occurred!"
  )
  print(result)
}
...
sum_digits <- function(n) {
  ...
  result <- helper(n)
  cat("Sum of the digits of", original, "=", result, "\n")
}
...
mtrx
double_mtrx <- apply(mtrx, 2, function(x) x * 2)
double_mtrx
```
...
deposit <- function(amount, balance = 500){
  if (amount < 0){
    stop("Deposit amount cannot be negtive!")
  }
...
sum_matrix <- function(m){
  ...
  progressBar <- txtProgressBar(min = 0, max = nrow(m) * ncol(m),
                                style = 2, char = "*")
...
count_vowel <- function(str){
  ...
  is_vowel <- function(ch) {
    ch <- tolower(ch)
    return(ch %in% c("a", "e", "i", "o", "u"))
  }
  count <- 0
  for (i in seq_along(strsplit(str, "")[[1]])){
    ch <- strsplit(str, "")[[1]][i]
    if (is_vowel(ch)) {
      count <- count + 1
    }
  }
  cat("There are ", count, " vowel(s) in '", str, "'", sep = "")
}
count_vowel("12e45a")
count_vowel("123456")
count_vowel("abcdea")
